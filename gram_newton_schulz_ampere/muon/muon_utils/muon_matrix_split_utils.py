    ...
    if param_split_fn is not None:
        ...
        for idx, ns_input in enumerate(ns_inputs_for_rank):
            ...
            if idx == 0:
                ...
                num_submatrices_per_input = (
                    len(sub_matrices) * tensor_dim_0_if_3d
                    if is_3d
                    else len(sub_matrices)
                )
            split_ns_inputs.extend(
                [
                    sub_matrix[i]
...
def scale_newton_schulz_outputs_with_adjusted_lr(
    ...
    """
    return {
        shape: batched_tensor.mul_(
            lr if adjust_lr_fn is None else adjust_lr_fn(lr, shape)
        )
...
def reconstruct_update_from_newton_schulz_outputs(
    ...
    orthogonalized_submatrices = [
        orthogonalized_by_shape[shape][idx] for shape, idx in shape_indices
    ]
    ...
    if param_recombine_fn is not None:
        orthogonalized = [
            param_recombine_fn(
                [
                    torch.stack(
                        orthogonalized_submatrices[i + j : i + j + tensor_dim_0_if_3d]
                    )
                    for j in range(0, num_submatrices_per_input, tensor_dim_0_if_3d)
                ]
                if is_3d
                else orthogonalized_submatrices[i : i + num_submatrices_per_input]
            )
            for i in range(
                0, len(orthogonalized_submatrices), num_submatrices_per_input
            )
        ]
    elif is_3d:
        orthogonalized = [
...
def validate_param_split_fn(
    ...
    fn_name = type(param_split_fn).__name__

    assert all(
        sub.ndim == ns_input.ndim for sub in sub_matrices
