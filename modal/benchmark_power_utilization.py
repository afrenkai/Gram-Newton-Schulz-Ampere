...
try:
    from gram_newton_schulz_ampere.kernels.cutlass_ns import (
        cutlass_symmetric_baddbmm_fused_mirror,  # ty: ignore[unresolved-import]
    )
except ImportError:
    cutlass_symmetric_baddbmm_fused_mirror = None
...
def main() -> None:
    ...
    primitive_operations: list[tuple[str, Callable[[], Tensor]]] = [
    ...
    fused_mirror_operation = cutlass_symmetric_baddbmm_fused_mirror
    if fused_mirror_operation is not None:

        def run_fused_mirror() -> Tensor:
            return fused_mirror_operation(
                accumulator,
                matrix.mT,
                matrix,
                beta=0.0,
        ...
        primitive_operations.append(("cutlass_fused_mirror", run_fused_mirror))
